...
import { initQuiz, submitAnswer, nextQuestion, getCurrentCar } from "@/lib/quiz-engine";
...
function getCodeExplanation(code: string, series: string): string {
  ...
  if (code.includes("LCI")) parts.push('"LCI" (Life Cycle Impulse) is BMW\'s term for a mid-cycle facelift update.');
  if (/M[345688]|1M/.test(code)) parts.push("M models are high-performance variants developed by BMW M GmbH.");
  if (series === "Z Series") parts.push('The Z designation stands for "Zukunft" (Future), BMW\'s roadster line.');
...
function QuizContent() {
  ...
  useEffect(() => { setQuizState(initQuiz(difficulty)); }, [difficulty]);
    ...
    setTimeout(() => {
      if (mode === "rounds" && newState.answers.length >= roundSize) {
        const params = new URLSearchParams({
          score: newState.score.toString(), total: newState.answers.length.toString(),
          difficulty, mode, bestStreak: newState.bestStreak.toString(),
        });
        router.push(`/results?${params.toString()}`);
        return;
      }
      setTransitioning(true);
      Promise.all([preloadPromise, new Promise((r) => setTimeout(r, 300))]).then(() => {
  ...
  function handleQuit() {
    if (!quizState) return;
    const params = new URLSearchParams({
      score: quizState.score.toString(), total: quizState.answers.length.toString(),
      difficulty, mode, bestStreak: quizState.bestStreak.toString(),
    });
    router.push(`/results?${params.toString()}`);
  }
            ...
            {quizState.answered && (
              <div className={cn(
                ...
                <div className="flex items-start gap-3 mb-3">
                  <div className={cn("p-2 rounded", isCorrect ? "bg-primary-container/20" : "bg-destructive/20")}>
                    <FileText className={cn("h-4 w-4", isCorrect ? "text-primary" : "text-destructive")} />
                  </div>
                  <div>
                    <h4 className={cn(
                      "text-[10px] font-mono tracking-[0.15em] uppercase",
                      isCorrect ? "text-primary-container" : "text-destructive"
                    )}>
                      {isCorrect ? "VERIFIED" : "INCORRECT"} CHASSIS INTEL: {currentCar.internalCode}
                    </h4>
